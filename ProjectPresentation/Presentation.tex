...
\title[CAVIR]{Cascading View Trees} % The short title in the optional parameter appears at the bottom of every slide, the full title in the main parameter is only on the title page
...
\begin{frame}
	\frametitle{Query Classes}
		...
		\begin{tabular}{lclclcl}
			& \textbf{Preprocessing} & \textbf{Delay} & \textbf{Update}\\
			\midrule
			\textbf{Q-Hierarchical} &$O(1)$ & $O(1)$  & $O(1)$ \\
			\textbf{Hierarchical}&$O(1)$ & $O(1)$  & $O(1)$ \\
			\textbf{Input Top} &$O(1)$ & $O(1)$  & $O(1)$  \\
			\textbf{Neither}&$O(1)$ & $O(1)$  & $O(1)$ \\
		\end{tabular}
		\caption{Performances of Query Classes}
	\end{table}
\end{frame}
\subsection{Cascading Example}
\begin{frame}
...
\begin{columns}[c] 
	...
	\begin{column}{0.45\textwidth} 
		\begin{figure}
			\includegraphics[height=0.6\textheight]{NonValidView.pdf}
...
\begin{frame}
	...
	Complete subtrees, combination of sibblings
\end{frame}

\subsection{Combining Queries Queries}
\begin{frame}
...
\end{frame}
\begin{frame}
	\frametitle{Complete Solutions}
	How to explain
\end{frame}
\begin{frame}
	\frametitle{Reverse Search}
	Compute resolving views for non-q-hierarchical queries \& find them in q hierarchical Queries
\end{frame}
...
\begin{frame}
	\frametitle{Greedy}
	Check all orderings; check some orderings?
\end{frame}
\begin{frame}
	\frametitle{Complete Solutions}
	How to avoid n! views for many similar relations?
\end{frame}
\begin{frame}
	\frametitle{Reverse Search}
	How?
...
\subsection{Characterisation of reducable Query sets}
\begin{frame}
	Ideas?
\end{frame}
\subsection{Non cascading q hierarchical optimizations }
\begin{frame}
	Take as base not only the q-hierarchical queries but any query
	use q-hierarchical subtrees
\end{frame}
...
\begin{frame}
	...
	\begin{figure}
		\includegraphics[width=0.8\linewidth]{dast-logo.png}
